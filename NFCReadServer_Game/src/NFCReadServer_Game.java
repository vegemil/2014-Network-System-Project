...
public class NFCReadServer_Game {
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		System.out.println("--------NFCRead Game Server Start!! -----");
	...
	private static void getAttendance() throws SQLException {
		...
		try {
			...
			if (st.execute("SELECT id, time FROM subject_game where date_format(time, '%Y-%m-%d') = '2014-06-11'")) {
				rs = st.getResultSet();				
				while (rs.next()) {
			...
			if (st2.execute("SELECT id, time FROM subject_game where date_format(time, '%Y-%m-%d') = '2014-06-12'")) {
			...
			if (st3.execute("SELECT id, time FROM subject_game where date_format(time, '%Y-%m-%d') = '2014-06-13'")) {
